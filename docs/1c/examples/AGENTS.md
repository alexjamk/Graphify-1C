...
Конфигурация и расширения — узлы графа; `contains` связывает их с объектами метаданных, модулями и методами. Для вопроса «все места использования» проходи страницы `--offset 0`, `100`, `200` до пустого ответа и проверяй все относящиеся к вопросу типы связей. Не загружай весь `graphify-out/graph.json` в контекст. `graphify-1c update .` обновляет граф 1С полным повторным анализом.
