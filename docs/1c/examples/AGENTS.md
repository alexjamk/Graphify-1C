...
2. Возьми точный `id` из результата. Запроси нужную группу связей, например `graphify-1c index neighbors graphify-out/graph.sqlite '<id>' --direction both --relation calls`. Вместо `calls` подставь требуемое отношение (`EXTENDS`, `AFTER`, `writes` и т. д.).
