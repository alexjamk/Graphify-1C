...
Если графа нет, создай его: `graphify-1c analyze ./src --out graphify-out/graph.json --html graphify-out/graph.html`. После изменения исходников выполни `graphify-1c update .`; при изменениях это полный повторный анализ 1С. Если после анализа SQLite отсутствует, выполни `graphify-1c index build graphify-out/graph.json graphify-out/graph.sqlite`.
