...
**Graphify-1C fork:** Analyze a 1C project with `src/cf` and `src/cfe/*` using `python -m graphify analyze ./src --out graphify-out/graph.json --html graphify-out/graph.html`. Direct Configurator XML and EDT exports also work. Install the optional parser with `pip install -e ".[onec]"`. See [1C architecture](docs/1c/ARCHITECTURE.md) and [current coverage](docs/1c/IMPLEMENTATION_STATUS.md).
