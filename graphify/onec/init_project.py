...
`graphify-1c`. Основная конфигурация — `src/cf`. Если манифест существует,
используй только записанный в нём выбор расширений. Если манифеста нет,
`analyze ./src` включит все корректные непосредственные каталоги `src/cfe/*`.
Когда пользователь выбирает только часть расширений, анализируй `./src/cf`
с повторяемым `--extension <путь>` для каждого выбранного каталога.

Сначала прочитай только `graphify-out/.graphify_onec.json`, если он есть.
Это маленький манифест текущего проекта: `root` указывает корень анализа,
`extensions` — явно переданные `--extension`, `out` — JSON, `html` — HTML.
Пустой `extensions` при `root=src` означает автоматическое обнаружение всех
корректных `src/cfe/*`, а не граф без расширений.
